...
class DailyTask(WWOneTimeTask, BaseCombatTask):
    ...
    def claim_daily(self):
        ...
        if total_points < 100:
            ...
            self.click_pending_daily_quest_reward()
            self.sleep(1)
            total_points = self.get_total_daily_points()
    ...
    def click_pending_daily_quest_reward(self):
        claim_buttons = self.ocr(0.78, 0.20, 0.96, 0.72, match='领取')
        if claim_buttons:
            claim_button = min(claim_buttons, key=lambda box: box.y)
            self.log_info(f'claim pending daily quest rewards via OCR {claim_button}')
            self.click_box(claim_button, after_sleep=0.5)
            return True

        self.log_info('claim pending daily quest rewards via fallback coordinate')
        self.click(0.88, 0.275, after_sleep=0.5)
        return False
    ...
    def click_daily_reward_box(self, reward_points):
        ...
        if reward_boxes:
            ...
            click_box = reward_box.copy(
                x_offset=int(reward_box.width * 0.3),
                y_offset=int(-reward_box.height * 1.8),
                width_offset=int(reward_box.width * 2.0),
                height_offset=int(reward_box.height * 2.0),
        ...
        self.click(0.925, 0.89, after_sleep=1)
        return False
